...
This Dockerfile will allow you to create or remove a GCP project. Security credentials
are expected to be stored on the Docker host desktop and can be made available
to the container with volume definitions. The output of this Dockerfile will include
a running instance of Apigee hybrid and a folder structure containing all of the 
files that have been configured for the deployed hybrid instance. Please note that
the folder structure is enabled so that command line work can continue without 
Ansible should it be required. 
